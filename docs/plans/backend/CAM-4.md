...
| `package.json` (edit)               | —                                                                                                      | dependency `@fastify/cookie` (latest v11.x at implement time, exact-pinned via `pnpm add --save-exact`).                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `test/support/http.ts` (new)        | `TEST_SESSION_SECRET`, `TestAppLayer`, `makeTestApp`                                                   | The new inject harness (root decision log): `TestAppLayer = ClockLive + IdGeneratorLive + repo layers + Layer.succeed(SessionSignerPort, makeSessionSigner(literal test secret))` over `TestDatabaseLive`; `makeTestApp(configOverrides?)` builds a hand-rolled `AppConfig` object (test DB, controllable TTL/attrs) + a silent pino logger + `ManagedRuntime`, awaits `buildServer`, returns `{ app, runtime }` for `afterAll` disposal. `db.ts` stays untouched — sibling helper, existing suites unaffected. |
...
grep -rn "node:crypto" packages/                       # crypto lives in apps/api only (C5.1)
...
| ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| C1.1   | _planned: use-case unit on stubs (M2) + HTTP inject 201-and-row check incl. trimming (M5)_                                                                                                                            | —                |
| C1.2   | _planned: use-case unit pins `expiresAt = now + ttl` and signed payload (M2); HTTP inject decodes the Set-Cookie via the test signer and checks attributes (M5)_                                                      | —                |
| C1.3   | _planned: HTTP inject 400-matrix — missing/empty/whitespace/33-char name ⇒ 400, no row, no cookie; 32-char boundary ⇒ 201 (M5). The contracts schema is pinned here, not by a contracts-package suite (decision 7)_   | —                |
| C2.1   | _planned: use-case valid-path unit (M2) + HTTP replay of the created cookie against `GET /me` ⇒ 200 same user (M5)_                                                                                                   | —                |
| C2.2   | _planned: HTTP inject `GET /me` with no cookie ⇒ 401 (M5)_                                                                                                                                                            | —                |
| C2.3   | _planned: adapter units — tamper + wrong-secret ⇒ typed `SessionInvalid` (M3); HTTP 401 for both (M5). Constant-time compare is by construction (`timingSafeEqual`) — disclosed as review-checked, not timing-tested_ | —                |
| C2.4   | _planned: use-case expiry unit (repo never consulted) (M2) + HTTP 401 for an authentic-but-expired token forged with the test signer (M5)_                                                                            | —                |
| C2.5   | _planned: use-case `UserNotFound` passthrough unit (M2) + HTTP 401 for unknown-uuid and soft-deleted-user cookies (M5)_                                                                                               | —                |
| C3.1   | _planned: use-case renewal unit (renewed token = sign(same user, now + ttl)) (M2) + HTTP fresh Set-Cookie on `GET /me` with later expiry (M5)_                                                                        | —                |
| C4.1   | _planned: `AppConfig` unit via `ConfigProvider.fromMap` — missing `SESSION_SECRET` ⇒ `ConfigError` (M3); manual boot-failure check recorded in Progress (M4/M5)_                                                      | —                |
| C4.2   | _planned: config-defaults unit (604800 / lax / not Secure) (M3); HTTP attribute assertions under default and overridden test config (M5); literal-leak greps in the M5 sweep_                                         | —                |
| C5.1   | _planned: structural — boundary lint in the gate + the `node:crypto`/`Date.now` greps print nothing; no dedicated test can pin an absence better than the linter that fails CI_                                       | —                |
| C5.2   | _planned: structural + indirect — contracts package builds importing only `effect`; the M5 HTTP suite round-trips both wire shapes through their encode/decode helpers_                                               | —                |
