...
- **Crypto:** `src/infra/ids.ts` holds the repo's only `node:crypto` import
  ...
  ban non-`effect` npm imports in `domain`/`contracts`/`application`
  `src/**`; **correction (review finding):** Node builtins are NOT caught —
  `eslint-plugin-boundaries` classifies them as origin `"core"`, which the
  `"external"` policy never matches, so `node:crypto` there lints clean.
  The builtin ban is grep-and-review-enforced pending a follow-up lint
  task. `test/**` blocks already permit vitest.
...
| `src/presentation/server.ts` (edit) | —                                                                                                               | Registers `@fastify/cookie` (before routes; parse/serialize only — no `secret` option, ADR-0018 §2) and `usersRoutes(options.runtime, options.config)`; sets a global `setErrorHandler` (fix cycle) so defects and framework errors on matched routes get a curated body via `unhandledErrorResponse` instead of Fastify's default `{ message: err.message }`. Unmatched routes (404) still use Fastify's default not-found body — echoes only the caller's own method/URL; `setNotFoundHandler` is CAM-5 material.     |
...
| `test/Auth.test.ts` (new)           | —                                                                                                               | The C1–C3 HTTP matrix through `app.inject()` (M5), plus the curated fallback-body test for the error handler (fix cycle) — 11 tests.                                                                                                                                                                                                                                                                                                                                                                                    |
...
`typecheck` + `lint` clean (lint pins the npm-package half of C5.1; the
builtin/`Date.now` half is pinned by the M5 grep sweeps — see the Surprises
entry on the boundaries plugin's `core` origin).
...
| C5.1   | structural, honestly split (review correction): npm-package imports are pinned by the boundary lint in the gate; Node-builtin imports and `Date.now()` are NOT lint-catchable (builtins are origin `"core"` to the plugin; `Date.now` is a global) — pinned by the grep sweeps above + review, until the follow-up lint task                                   | `node:crypto` imports exist only under `apps/api`; no `Date.now`/`new Date(` in `packages/application/src` — greps verified at review, not CI-enforced                                                                                                |
...
- [x] 2026-09-01 12:40 — Fix cycle (review findings 1–3): docs corrected to
      state the real C5.1 enforcement split (builtins/`Date.now` are grep-
      and-review-pinned, not lint-pinned — the boundaries plugin treats Node
      builtins as origin "core"); `setErrorHandler` added in `server.ts`
      with `unhandledErrorResponse` in `errors.ts` (client-fault statuses
      kept, bodies curated, error logged server-side); `POST /users` now
      routes its failure through `sessionErrorStatus`. New Auth test pins
      the malformed-JSON path → 400 `{ error: "bad request" }`, no
      `message` field. Api suite: 47.
...
- Implement-time note: the planned `grep -rn "node:crypto" packages/` sweep
  ...
  for npm packages only — see the next bullet.
- **Review discovery (C5.1):** the boundaries plugin does not classify Node
  builtins as `"external"` (they are origin `"core"`), so the effect-only
  policy never fires on them — `import "node:crypto"` in
  `packages/application/src` lints clean. Verified by probe during review.
  Until the follow-up lint task closes this, the builtin ban is enforced by
  the grep sweeps and review, and the plans/coverage rows now say so.
