...
'30 days')`** — games first, then users (root Decision Log ordering),
   one captured timestamp so a game and its dependents carry the identical
   tombstone (C3's "single call" clause; a plpgsql variable or CTE over
...
- **C2**: expire a lobby out from under a held `expectedVersion`; the
  ...
  `'abandoned'`.
- **C3**: a completed game backdated 31 days; one soft-delete call
  tombstones the game and every dependent row across all five tables with
  the identical `deleted_at` timestamp.
