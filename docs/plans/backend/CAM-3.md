...
| C1.3   | `Migrations.test.ts` — "game_players/decks keys match the contract" + "every unique constraint is partial…"                                          | PK is (game_id, user_id); `game_players_seat_key` indexdef contains `WHERE (deleted_at IS NULL)`; columns incl. final_score/is_connected/is_bot                                 |
...
| C1.5   | `Migrations.test.ts` (partial uniques) + `RoundTrip.test.ts` C5.3 partition + mid-flight load equality                                               | both user_cards uniques are partial; persisted rows are exactly the occupied slots of a sparse hand (holes survive the round-trip uncompacted)                                  |
...
| C5.3   | `RoundTrip.test.ts` — the five "§4.5 … verbatim against persisted rows" tests                                                                        | 52-slug partition over decks.cards+user_cards+discard_pile(+phase-held), counts ≥ 0, seat_index = [0..n−1], Σfinal_score = Σscore(card), Ended⟺completed — all by SQL over rows |
