...
| 8      | app `RoomRegistry.test.ts` “slam race: first-in-queue wins, deterministically, over 20 fresh rooms (clause 8)”                                                                                                                                 | 20 fresh-room races: identical (replyA, replyB, event-log) triple every time; first-enqueued slam succeeds; second’s outcome asserted stable, not invented                                                                                                                                                                                                            |
...
| 10     | app `RoomRegistry.test.ts` “eviction: a finished game's next command gets a fresh bootstrap and a typed error (clause 10)” and the abandoned-lobby twin                                                                                        | after `Ended`, journal shows a fresh bootstrap `load` and the reply is `GameAlreadyEnded`; join after abandonment is `LobbyNotJoinable`; no crashes                                                                                                                                                                                                                   |
| 11     | app `RoomRegistry.test.ts` “timer vs lazy close are equivalent, and a stale timer is a silent no-op (clause 11)”                                                                                                                               | lazy world (TestClock never advanced) journals close batch then probe; timer world (TestClock adjusted) yields identical reply, state, and event log; stale timer adds zero journal entries                                                                                                                                                                           |
