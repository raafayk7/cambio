...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| S1 — arm on load                   | packages/application/test/RoomRegistry.test.ts — "poke closes a cold, past-due window — bootstrap, judge the clock, close (S1+S3+S5)"; packages/application/test/SlamTiming.test.ts — "restart mid-window: the late slam is refused and the window closes exactly once before the next command (C4.2)" | bootstrap arms a timer for a loaded SlamWindow; a past-due one closes promptly                                                                         |
...
- [x] 2026-09-07 — Step 2 done: `Poke` envelope, `registry.poke`,
      ...
      `packages/application/src/room/RoomRegistry.ts`. TDD followed: 7 new
      tests written first in `packages/application/test/RoomRegistry.test.ts`
      (1 extended VersionConflict test + 1 new SlamWindow-conflict variant +
      5 new Poke tests), confirmed they exercised the not-yet-existing
