...
| 11. State lifecycle                  | New or extended SL3 case, alongside the existing `SlamWindowClosed`+`TurnAdvanced` batch test — ready-mode/armed state resets on the same phase transition `selection`/`slamPendingGive` already reset on. | game-screen.test.tsx > "(CAM-23) ready-mode and an armed give-slot reset the moment the acting phase moves on from SlamWindow"                                                                                                                                                                  | after an armed give-slot is set, a SlamWindowClosed+TurnAdvanced batch clears both the Cancel give and Ready a give controls                                                    |
