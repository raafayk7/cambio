...
- **File to edit:** `apps/web/src/containers/lobby/lobby-screen.tsx`.
  `LobbyScreen()` (line 42) builds a `content` variable (lines 71-152)
  ...
  `<AppShell scene="courtyard" ...>` (line 155) wrapping
  `<div className="mx-auto flex w-full max-w-md flex-1 flex-col justify-center gap-5 p-5">{content}</div>`
  (lines 156-158), followed by `<HowToPlayGuide .../>` (line 159). `Link` is
  not yet in the `@cambio/ui` import list at the top of this file (currently
  `Alert, AppShell, Button, FieldScaffold, Panel, Skeleton, TextField`).
...
| --------------------------------------------------------------------------------------------- | ------------------ | ---------------- |
| 1. Exactly one `Link` with the exact copy, href, target, rel                                  |                    |                  |
| 2. Present in all four lobby states (outside the branching `content`)                         |                    |                  |
| 3. Sibling to `{content}` inside the existing `mx-auto max-w-md` column; no `AppShell` change |                    |                  |
| 4. No new design-system component/token/icon; wrapper classes are spacing-only                |                    |                  |
| 5. Existing lobby-screen tests continue to pass unmodified                                    |                    |                  |
