...
- [x] 2026-09-06 — M1 (tokens): compact card-lg/card-md scale and
      ...
      tuned during M5 (see Surprises). The ui suite ran green via
      `pnpm turbo test` filtered to `@cambio/ui`.
...
- [x] 2026-09-06 — M5 (rendered pass): design-gate `render.js` at
      ...
      room screen unaffected (rendered + jsdom). _(Correction, review
      F1: the room-screen claim was false — the compact art cap and the
      root gap-2 reached the room screen through TableSurface's shared
      root, halving the room's table art at compact; scoped to
      `viewerSeat="external"` in the fix cycle.)_
...
- [x] 2026-09-06 (review fix cycle) — `table-surface.tsx`: art cap +
      root gap keyed to `viewerSeat === "external"` (F1);
      `game-screen.tsx`: own-seat rest `regular:hidden` +
      `data-region="own-seat-rest"` (F2), regular restorations
      (`regular:flex-initial`/`regular:min-h-auto`), comment fixes,
      `regular:order-none` dropped; `game-screen.test.tsx`: 4 assertions
      strengthened + 1 new test (68 total), ADR-0035 guard widened;
      canon version bumps + amendments (playing-card r3 among them);
      styles.css art-cap rationale. All findings' branches recorded in
      the root plan's retrospective.
...
- M5 (2026-09-06): the initial fold-budget numbers (art 160px, card-lg
  ...
  before and after. _(Amended at the review fix cycle: the "2p/3p/4p
  fit" measurement proved NAME-WIDTH-SENSITIVE — the review's 4-player
  probe game, seat pills carrying 6-character names, wraps the opponent
  row into the sanctioned middle scroll (110px overflow, identical
  pre- and post-fix-cycle, so not a fix regression). See the root
  plan's twice-amended acceptance criterion for the guarantees that
  hold at every count.)_
