...
- Motion tokens: `--duration-snap: 140ms`, `--duration-track: 340ms`,
  ...
  does not exist — tokens.md §Motion (at plan time; since revised r2) says
...
12. **Powers + peeks (T3, T4).** Targeting via the extended Hand
    ...
    (CH1). Non-holders see the seat acting only (phase carries no card
    for them; the power goes public at `PowerDiscarded` — review F2
    amendment). jsdom tests: reveal appears, disappears
...
| C4     | ledger above (built states each pinned by their branch tests; N/A rows carried by justification, partial-failure conflict surfaced)                                                                                                                    | the Page-state ledger in this document; built states pinned by `apps/web/test/game-screen.test.tsx` branch tests (bootstrap/skeleton/no-access/NameForm-in-place/page-error/reconnecting describes)                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | all 8 states answered: 5 built with branch tests, 3 N/A with justifications; the partial-failure canon conflict surfaced, not skipped                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| C5     | hidden-info sweep: face-down DOM carries no value; only wire-delivered values render; peeked values gone after the window; recorded calls limited to view GET + commands POST                                                                          | `apps/web/test/game-screen.test.tsx` — "renders every hand slot face-down with no card value anywhere in the DOM", "makes no requests beyond the view GET and /me", "PenaltyDrawn never renders a card value, even once its reveal-gated flight lands (C5 extension, ADR-0022)", plus the T4 never-re-renders pins                                                                                                                                                                                                                                                                                                                                                              | face-down DOM value-free; wire calls limited to view GET + commands POST (+ /me); penalty values never exist client-side; peeked values gone after the window and never re-rendered                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| T3     | power targeting per class incl. J two-distinct-slots and the Queen two-step (`PowerPeek` then `PowerSwap`); fizzle renders a public beat; non-holder sees the seat acting only (F2 amendment)                                                          | `apps/web/test/game-screen.test.tsx` — "9/10 target one opponent occupied slot with PowerPeek", "J requires two distinct occupied slots before sending PowerSwap; a repeat click deselects", "the Queen's two-step sends PowerPeek, then (once the phase moves on) PowerSwap for two distinct slots", "PowerFizzled renders the public fizzle copy, then clears" (review F5); 7/8 peek-own targeting in `affordances.test.ts` per-phase describes                                                                                                                                                                                                                               | targeting per power class with selection state; J two-distinct rule; Queen two-step command sequence; the fizzle beat renders and clears; no decline affordance exists anywhere                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| T4     | peek reveal state machine with fake timers: flips up for `duration.peek`, flips back, never re-renders, value absent from DOM after; Queen's value clears before swap selection                                                                        | `apps/web/test/game-screen.test.tsx` — "reveals the peeked card, then clears after the peek duration and never re-renders", "the Queen's swap-picking stays disabled while the reveal is showing, then enables once it clears", "a public CardPeeked marks the peeked slot with the selected treatment for one reveal beat — no value anywhere" (review F3)                                                                                                                                                                                                                                                                                                                     | PrivateCardPeeked flips the target for PEEK_DURATION_MS (2800, --duration-peek) then clears permanently; the Queen swaps from memory; the PUBLIC half shows which slot was peeked for one duration.reveal beat, value-free (review F3 strengthening)                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| E1     | CambioCalled flips the indicator to game-over copy before any reveal renders (ordering pinned structurally)                                                                                                                                            | `apps/web/test/game-screen.test.tsx` "the call moment (E1)" — "flips the turn indicator to the CAMBIO! call copy the instant CambioCalled arrives — before any reveal renders", "prefers the ephemeral announcement's own player over a stale turn-active seat ring until the snapshot catches up"                                                                                                                                                                                                                                                                                                                                                                              | ephemeral `calledBy` (use-game.ts) flips the indicator's `data-state` to `game-over` and its copy to "<Name> called CAMBIO!" synchronously on the room event, strictly before the debounced refetch could land a real `Ended` view — asserted by checking no reveal/score-sheet content exists yet and only one `data-state="game-over"` element (the indicator) is on the page; names the actual caller even when a different player was mid-turn                                                                                                                                                                                                                                                          |
| E2     | caller renders reveal from their own GameReply; non-caller refetches on GameEnded; score sheet sorted ascending, plural tie winners, true-minus totals, caller unmarked, revealing entrance state present                                              | `apps/web/test/game-screen.test.tsx` "the reveal (E2)" — "renders the settled score sheet: sorted ascending, plural tie winners, true-minus totals, caller unmarked, zero-card hand not styled as winning", "dims the table under the game-over state while the score sheet is showing", "a fresh bootstrap straight into an Ended view renders the score sheet, not an error — the revealing entrance is structurally present"; `apps/web/test/score-sheet.test.tsx` "the revealing entrance (r2)" — "defaults to already-settled (final) when revealing is omitted", "mounts face-down and revealing when asked to dramatize the entrance, then settles to face-up and final" | ascending order survives a fewer-cards-but-higher-total zero-card hand; both tied winners marked, the zero-card hand isn't; `−1` (true minus) rendered twice; the caller's row is class-identical to the other winner's (no distinct caller marker); `ScoreSheet`'s new `revealing` prop mounts `data-state="revealing"` with every mini card `face="down"`, then a `duration.track`-timed flip settles `data-state="final"` with faces up — asserted structurally (state attribute + `data-face`), never as animation; the caller's own `GameReply` path was pre-existing (version-guard `onSuccess`) and needed no new code — the E2 gate confirmed only, per the deliverable's "verify, don't duplicate" |
| E3     | game-over dims the table under the sibling-overlay composition; single back-to-lobby exit navigates `/`; a fresh mount on an ended game renders the score sheet, not an error                                                                          | `apps/web/test/game-screen.test.tsx` "game-over composition and exit (E3)" — "offers exactly one exit — back to the lobby — which navigates to /" (also see the E2 "dims the table" and "fresh bootstrap" tests above, which double as E3 coverage for the composition and reload cases)                                                                                                                                                                                                                                                                                                                                                                                        | exactly one `role="link"` named "Back to the lobby" exists while the sheet shows, and clicking it drives `router.state.location.pathname` to `/`, landing on the harness's `index-route-stub`; `TableSurface` takes `state="game-over"` (asserted via the seat wrapper's nearest `[data-state]` ancestor) while the sheet renders as a sibling overlay in the same `relative` table-root div, after `FlightLayer` in DOM order, never inside `center`                                                                                                                                                                                                                                                       |
| CH1    | flight specs enqueued for each public movement (draw, discard placements, blind swaps both directions, gives, penalties, slam removals) with deck `draw` / pile `receiving` states during; movement itself verified in the rendered path + walkthrough | `apps/web/test/flight-layer.test.tsx` structural pins (value-free spec renders a back, entitled spec a face, clock-settle tests — review F1) + component-level state pins (`draw-deck.test.tsx`, `discard-pile.test.tsx`); the `handleRoomEvent` enqueue wiring itself has NO automated pin (review F5 correction — jsdom auto-cancels degenerate plans synchronously, so interaction tests observe commands and display state, never enqueues); enqueue wiring verified by the rendered path + live walkthrough                                                                                                                                                                | flight faces are entitlement-structural and flights settle by the clock; choreography states render at component level; the event→enqueue wiring is rendered-path/walkthrough evidence only                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| CH3    | reduced-motion branch covered structurally per flight kind (highlight treatment renders, no overlay card); full parity verified in the rendered pass (step 15)                                                                                         | `apps/web/test/flight-layer.test.tsx` — "reduced motion renders origin/destination highlights, never a moving card"; rendered path: gallery render under emulated `prefers-reduced-motion` (gate run, step 15) clean                                                                                                                                                                                                                                                                                                                                                                                                                                                            | the reduced-motion branch renders the cross-fade + accent.focus treatment for every flight kind; no information exists only in motion                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
- [x] 2026-09-05 20:30 — step 14 (M6, E1–E3) complete: `use-game.ts`
      gained the ephemeral `calledBy` (set on `CambioCalled`, also on
      `GameEnded` belt-and-suspenders for a client that missed the
      former), consulted by `game-screen.tsx` only while
      `view.phase._tag !== "Ended"` — once the refetched snapshot
      actually reaches `Ended`, `turnStatus` reads `phase.calledBy`
      directly and the ephemeral value stops mattering (no separate
      cleanup timer needed). The game-over turn-indicator copy is the
      second poster-caps shout moment (voice.md): "<Name> called
      <span class="font-display">CAMBIO!</span>" — the name stays
      sentence case per turn-indicator.md's own example, "CAMBIO!" gets
      the display-face + `!` treatment `SlamTimer`'s "SLAM!" already
      uses. `ScoreSheet` gained the `revealing` entrance (score-sheet.md
      r2): a `revealing` prop (default off — the pre-existing suite's
      immediate-`final` behavior is unchanged), backed by one
      `window.setTimeout(SCORE_REVEAL_MS = 340)` mirroring
      `--duration-track` — mounted cards render `face="down"` and the
      root carries `data-state="revealing"` until the timer flips every
      card to `face="up"` and settles `data-state="final"`, never
      re-arming on a later re-render. `game-screen.tsx`'s `GameTable`
      derives `ended = view.phase._tag === "Ended"`, passes
      `TableSurface state={ended ? "game-over" : "in-game"}`, and — only
      once `view.reveal` is structurally present — renders `ScoreSheet`
      as an absolutely-positioned sibling overlay inside the same
      `relative` table-root div, placed after `FlightLayer` in DOM order
      (both `z-index:auto`, so later DOM order alone paints it on top —
      no `z-index` needed, though `z-30` was added for readability/
      robustness against future ordering changes) — never passed into
      `center`, per the plan's decided composition. The overlay's only
      other content is the single back-to-lobby exit (`Button asChild`
      wrapping a `RouterLink to="/"`, mirroring the no-access panel's
      existing `UiLink`+`RouterLink` idiom). Residual T3/T5 ephemera
      (`commandError`, `fizzleMessage`, `slamBeatMessage`) are
      additionally guarded with `!ended` — belt-and-suspenders, since
      the phase-keyed affordance checks already gate the Call
      Cambio/HoldingCard/slam UI to nothing once `affordances.phase`
      becomes `"Ended"`, and the `phaseKey` render-time reset already
      clears `selection`/`slamPendingGive` on the same transition. No
      changes were needed to the caller's own-`GameReply` reveal path
      (T5-era version-guard `onSuccess` in `use-game.ts` already applies
      it — verified by the "renders the settled score sheet" test using
      a bootstrap `GET` directly into `Ended`, which exercises the exact
      same apply-if-newer code path a `GameReply` would). 8 new tests:
      6 in `game-screen.test.tsx` ("the call moment (E1)" ×2, "the
      reveal (E2)" ×3, "game-over composition and exit (E3)" ×1) + 2 in
      `score-sheet.test.tsx` ("the revealing entrance (r2)" ×2, one
      pinning the omitted-prop default). `score-sheet.md` → r2. web 185
      (177+8), ui 25; filtered build+typecheck+lint+test green — bare
      `pnpm turbo build typecheck lint test` filtered to both packages,
      9/9 tasks, web 185/185, ui 25/25.
- [x] 2026-09-06 02:00 — step 15 (M6 close) part 1, verification +
      audits: full forced gate 25/25 (0 cached; api 118, domain 194,
      application 86, contracts 11, config 16, ui 25, web 185 = 635).
      **Live two-session walkthrough executed** (in-app browser as
      Raafay + cookie-jar HTTP session as Nadia): create → join-on-visit
      → start → full game to a Cambio call — draws both ways, every
      DEALT power class live (9/10 peek ×2, Queen full two-step with a
      cross-player swap, J two-pick blind swap from the browser; no 7/8
      was ever drawn), a failed slam → penalty with `closesAt` fixed
      through the attempt (ADR-0011 live), take-discard suppression
      verified in both directions on real power/non-power tops,
      discard-source hold correctly offering no discard-back, realtime
      kill/restart mid-game showing the in-game reconnecting treatment
      and a correct table on recovery, confirm-modal Cambio call →
      CAMBIO! indicator → score sheet byte-identical to the server
      reveal (winner marking, ascending order, true-minus) → back to
      lobby, and an ended-game reload re-rendering the sheet. Audits:
      `ai-tells` **1/30 (invisible)**, one actionable finding (error-copy
      suffix five-peat — fixed below); impeccable deterministic detector
      clean on all seven changed UI files; design-gate pipeline
      (decompose→map→judge on live authenticated renders + hard
      checks): hard checks PASS on mid-game, ended, AND compact
      (390×844) states — 0 constraint fails, sole advisory the sr-only
      h1; verdict **flagged, 6 accidental default-tier points, 0
      blocking**, plus 5 findings judged controlled/allowed
      (hands-over-benches, face-down hands at ended, score-row stagger,
      wide-viewport flanks, deck-under-panel).
- [x] 2026-09-06 02:45 — step 15 part 2, gate fix cycle (all six
      flagged points resolved, re-rendered, re-tested): (1+2 collisions
      & fold) `seatAnchor="edge"` on TableSurface (table-surface.md r3)
      anchors non-viewer seats at the ring edge growing inward — the
      shared `inwardSide` helper in table-geometry.ts now drives both
      the anchor and the seat/hand stacking order (the dead `handArc`
      call site is gone); the viewer's dock stays centered (its lg
      hand grown inward would cover the deck/discard — caught in the
      first re-render and reverted); the Call Cambio affordance moved
      out of the top band to dock at the stage's bottom-right at
      regular (the viewer's action, by their hand), which with the
      p-4/gap-4 trim (also clearing the flagged off-scale 20px padding)
      brings the mid-game own seat inside the fold and leaves the
      indicator alone in the top band — both renders confirm zero
      occlusion; (3 scrim) game-over adds a full-region `green-deep`/35
      rest above the seat layer, below the score overlay
      (table-surface.md r3) — the ended render now visibly
      subordinates everything; (4 grouping) ScoreSheet gained a
      `footer` slot (score-sheet.md r2 amended) and the back-to-lobby
      button renders ON the panel under the row hairline; (5 phantom
      slot) Hand's even-rounding filler cell renders an invisible
      spacer, never the dashed vacancy (hand.md r2 amended); (6 badge
      clearance) deck-discard gutter 12→16px. Also from the audits: the
      error-copy five-peat varied per voice.md's error formula, and the
      game-over "CAMBIO!" dropped `text-shadow-poster` (it overflowed
      the pill and muddied at 15px — display face alone reads clean).
      Suites green after each round (final web 185, ui 25,
      typecheck+lint+format clean); hard checks re-run on the fixed
      renders: PASS, sole sr-only advisory.

- [x] 2026-09-06 03:30 — review fix cycle (F1–F7, from the root
      retrospective): flight settle-by-clock + `useFlights` ref-based
      purity (F1/F7, 2 new tests); public which-slot-was-peeked beat
      (F3, 1 new test); fizzle + DrawSkipped beat tests (F5, 2 new);
      `duration.reveal` 1200ms minted (F6, user-approved — tokens.md
      r3, slam-timer.md r2, `REVEAL_DURATION_MS` rename); T3 claim
      amended by sweep (F2); doc batch (F4) incl. a fourth hand.md
      instance the finding's list missed. web 190 (185+5), ui 25;
      suites + full gate re-verified fresh below.
...
- **(step 15) A stale dev server nearly shipped a phantom finding.** The
  long-running Vite dev server (started before CAM-17's touch-floor
  landed) served a pre-`touch-floor` transform of `button.tsx` — the
  live DOM measured 42–43px controls while source and the compiled CSS
  carried the 44px floor. The gate's Map stage initially recorded D10/D6
  violations for it; a source-vs-served diff exposed the artifact, the
  mappings were withdrawn on evidence, and a fresh server measured ≥44px
  with the touch-target advisory gone. Lesson for future rendered-gate
  runs: verify the dev server's transform freshness (serve the module
  and grep it) before treating a rendered/declared mismatch as a code
  defect.
- **(step 15) The gate's constraint tier is structurally blind to
  occlusion.** The most severe defect found (the seat pill 100% covering
  the Call Cambio label) measured PERFECT contrast and hit-area — an
  occluded element still passes every static check. The judge flagged
  this explicitly; if verdicts ever stop being advisory, an
  occlusion/intersection hard check on `role=status` and primary-action
  elements is the missing check.
- **(step 15, follow-up) 3–5-player seat geometry is tuned for heads-up
  only.** The edge-anchor arrangement was verified live and rendered for
  2 players; left/right seats (4–5 players) grow inward toward the disc
  and may crowd the center cluster — the rendered tuning pass for those
  counts is future work (the fold/occlusion invariants hold by
  construction, but spacing needs eyes when multi-seat games get played).
- **(step 15, accepted) The ended state still overflows the fold by the
  viewer's fifth-card row + pill** (a 5-card hand is a 3-row grid). Left
  as an authored scroll: at game-over no countdown is running, the score
  sheet — the only actionable surface — is fully in-frame, and the
  overflow tail is the viewer's own (face-down) cards. The mid-game
  state, where the real-time constraint bites, fits the fold.
