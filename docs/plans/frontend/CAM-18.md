...
| CH1    | flight specs enqueued for each public movement (draw, discard placements, blind swaps both directions, gives, penalties, slam removals) with deck `draw` / pile `receiving` states during; movement itself verified in the rendered path + walkthrough | enqueue coverage rides the interaction suites (T2 draw/discard flights, SL2 give/penalty/removal flights in `game-screen.test.tsx`) + `flight-layer.test.tsx` structural pins (value-free spec renders a back, entitled spec a face); movement itself verified in the rendered pass + live walkthrough (jsdom cancels degenerate-rect flights — Surprises)                                                                                                                                                                                                                                                                                                                      | every public movement enqueues a flight with entitlement-structural faces; deck `draw`/pile `receiving` states pinned at component level (`draw-deck.test.tsx`, `discard-pile.test.tsx`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
