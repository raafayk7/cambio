...
- **Held card** — rendered in the table center at `game-screen.tsx:788-800`
  ...
  `design-system/components/core/held-card.md` (r1 then; r2 added the
  hint, r3 retired it). **Now implemented (as amended)**: the hint
  renders in the **chrome band**, not under the held card — the
  user-directed live-play round relocated it and retired the `hint`
  prop (root F3.1 amendment; step 13's placement paragraph is
  superseded).
...
**Step 13 — holder hint under the held card (F3, root D5/D7).**

> **Amended (review fix cycle):** as-built, the hint renders in the
> **chrome band** (`ink.primary`/semibold) and `HeldCard` has no `hint`
> prop — the user-directed live-play round relocated it after `ink.muted`
> proved near-unreadable on the table felt (root F3.1 amendment, D5
> superseded, held-card.md r3). The derivation below (targeting-keyed
> strings, holder-only, per-render) shipped exactly as planned; only the
> placement paragraph and the `hint`-prop mechanics are superseded.
...
`apps/web/test/held-card.test.tsx` _(as amended: that test was added in
M4 and deleted with the prop in the relocation — the chrome-band tests
above are the surviving pins)_. **The existing hidden-information
...
| F2.1   | apps/web/test/game-screen.test.tsx — "opens the guide from the game screen's help icon-button and closes it"                                                                                                                                                                                                                                                                                                         | Modal dialog opens titled "How to play"; all eight section headings (the seven F2.1 sections plus the r2 "About Cambio" intro) render inside it                                                                                   |
| F2.2   | apps/web/test/game-screen.test.tsx — "the guide's rules copy matches canon on the load-bearing spot-pins (F2.2)"; full-breadth fidelity verified by /review's line-by-line diff against cambio-rules + the cited ADRs                                                                                                                                                                                                | no-opening-peek statement, suit-split king scores with the true minus sign, the obligatory-power statement, rank-not-score slam wording, and the J/Q same-player allowance (fix cycle) all render verbatim inside the open dialog |
| F2.3   | apps/web/test/game-screen.test.tsx — "never offers or implies a card-tracking aid, and opening it makes no new requests (F2.3)"                                                                                                                                                                                                                                                                                      | the rendered guide states remembering is the game AND matches no tracking vocabulary (negative sweep, fix cycle); opening it adds no HTTP requests beyond the harness's recorded bootstrap set                                    |
...
| F3.1   | apps/web/test/game-screen.test.tsx — "holder sees the %s hint in the chrome band (F3.1)" (parameterized: 7H, 8H, 9H, TH, JH, QH) and "the guide's powers table carries exactly the F3.1 hint strings (no-drift pin)" _(the held-card.test.tsx hint test was deleted with the prop in the relocation — F3.1 amendment)_                                                                                               | each targeting kind renders its exact F3.1 spec string, scoped to the chrome band; the guide's powers table carries the identical four strings (no-drift pin)                                                                     |
...
| F3.3   | apps/web/test/game-screen.test.tsx — "the hint disappears once the phase leaves ResolvingPower (F3.3)"                                                                                                                                                                                                                                                                                                               | the hint string is absent from the chrome band once a broadcast + refetch move the phase to AwaitingDraw — it derives per render and persists nowhere                                                                             |
| F3.4   | apps/web/test/game-screen.test.tsx — "non-holder renders no hint and the indicator reads the power-card copy (F4.1, F4.3)"; pre-existing and untouched: apps/web/test/affordances.test.ts "non-holder: nothing (no card field — pinned by ViewFor.test.ts)" and packages/application/test/ViewFor.test.ts; the "hidden information (C5, structural sweep)" describe in game-screen.test.tsx passes byte-unchanged    | the chrome band carries no F3 hint string for a non-holder; every existing hidden-info structural pin still holds                                                                                                                 |
...
| F4.2   | both F4.1 non-active tests (ResolvingPower and, since the fix cycle, ResolvingQueenSwap) assert data-state="other-turn" on the indicator via document.querySelector('[role="status"][data-state]'); TurnIndicatorProps's 4-value union is unchanged, held by typecheck + this review                                                                                                                                 | no new indicator state value exists; the power-card copy rides the existing other-turn state                                                                                                                                      |
| F4.3   | asserted structurally alongside F4.1 — apps/web/test/game-screen.test.tsx "non-holder renders no hint and the indicator reads the power-card copy (F4.1, F4.3)" scopes a negative assertion for any F3 hint string; the fixture deliberately carries a card field and the client still derives nothing from it (stronger than the wire, which omits the field — ViewFor.test.ts; comment corrected in the fix cycle) | the non-active copy names no rank and no power — the copy path never reads a card field, proven even against a fixture that supplies one                                                                                          |
...
| F5.5   | verified by review, no test — held-card.md r2 (hint added) + r3 (hint retired to the chrome band); root F5.5 amendment records the as-built state _(row added in the fix cycle — review Finding 2b)_                                                                                                                                                                                                                 | doc review                                                                                                                                                                                                                        |
...
- [x] 2026-09-08 — review fix cycle (root plan retrospective is the
      finding list of record): relocation amendments folded into Context,
      step 13/14, and Surprises; coverage table reconciled to as-built
      test names (F3.1 recited, F5.5 row added, F2.1/F2.3/F4.2/F4.3
      cells corrected); F1.3/F2.2/F2.3/F4.2 tests strengthened and the
      powers-table↔hint no-drift pin added; copy corrections per
      retrospective Findings 3 and 10. Suites re-run fresh — green.
...
- **held-card.md r2 is a fourth canon-doc touch** beyond root F5's
  ...
  _(As amended: the live-play round then retired the prop — held-card.md
  now carries r2 AND r3, and the "instruction copy, not an affordance"
  characterization applies to the chrome-band hint; root F5.5 amendment.)_
