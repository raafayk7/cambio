...
export const PlayerGameView = Schema.Struct({
   ...
   * runs on. Mirrors the `GameStarted` event's `config` field.
   */
 ...
 * at the table. `name` is 1–32 chars by construction at `POST /users`;
 * responses carry it as plain `Schema.String` (the `SessionUser` precedent).
 */
