...
import { makeGameRepoStub, makeIdsStub, makeJournal, makePublisherStub, opsOf } from "./support/stubs.js"

describe("createLobby (clause 1)", () => {
  it.effect("persists a one-member open lobby under the minted id, then publishes (clauses 1, 7)", () => {
    ...
    return createLobby({ creatorId: uid(0) }).pipe(
      ...
      Effect.provide(Layer.mergeAll(makeIdsStub(gid), repo.layer, makePublisherStub(journal).layer)),
    )
  })
})
