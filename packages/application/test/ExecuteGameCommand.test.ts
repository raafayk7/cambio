...
describe("executeGameCommand (clauses 5–7)", () => {
  it.effect(
    "a legal command: whole batch in ONE save, then ONE publish with the post-command state",
    () => {
      ...
      return seedGame(state).pipe(
        ...
        Effect.map((result) => {
          ...
          const published = journal[2]
          if (published?.op === "publishGame") expect(published.events).toEqual(result.events)
          const saved = journal[1]
          if (saved?.op === "save") expect(saved.events).toEqual(result.events)
        }),
  ...
  it.effect(
    "an illegal command is its typed GameError; nothing persisted, nothing published",
    () => {
      const journal = makeJournal()
      const repo = makeGameRepoStub(journal)
  ...
  it.effect(
    "a stale cached version surfaces the repo's VersionConflict unchanged; no publish",
    () => {
      const journal = makeJournal()
      const repo = makeGameRepoStub(journal)
      const state = dealt()
